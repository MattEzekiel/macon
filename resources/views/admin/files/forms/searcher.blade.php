...
@if(count($fields) > 0)
    <search>
        ...
        <div class="lg:hidden mb-4">
            <div class="collapse bg-base-200">
                ...
                <div class="collapse-content">
                    <form action="{{ route('admin.files') }}" method="get" class="flex flex-col gap-2.5 mt-1.5">
        ...
        <!-- Filtros para pc -->
        <form action="{{ route('admin.files') }}" method="get" class="hidden lg:flex flex-wrap gap-2.5 items-center mt-1.5">
            @foreach($fields as $key => $value)
                @if($key === 'client' || $key === 'product')
                    <div class="flex-1">
                        @php
